...
const Orders = () => {
  ...
  const handleReviewClick = (e, productId) => {
  ...
  return (
    <div className={`${theme?.text} min-h-screen`}>
      <div className="max-w-6xl mx-auto px-4 py-12">
        ...
        {orders.length === 0 ? (
          <div
            className={`${theme?.card} ${theme?.border} rounded-lg p-8 shadow-md text-center`}
          >
                ...
                >
                  <div className="flex flex-col space-y-1">
                    <div className="flex items-center">
                      ...
                      <span
                        className={`${statusStyles[order.status].bg} ${
                  ...
                  <div className="flex items-center space-x-4 mt-2 md:mt-0">
                    ...
                    {order.status === "delivered" && (
                      <button 
                        onClick={(e) => {
                          e.stopPropagation();
                          // Navigate to review page with the first product in the order
                          // You could modify this to show a dropdown if multiple products exist
                          if (order.items && order.items.length > 0) {
                            navigate(`/review/${order.items[0].productId}`);
                          }
                        }}
                        className="flex items-center bg-yellow-100 text-yellow-700 hover:bg-yellow-200 transition px-3 py-1 rounded-full mr-2 text-sm"
                      >
                ...
                {expandedOrder === order._id && (
                  <div className="border-t border-gray-200 dark:border-gray-700">
                    {order.trackingInfo?.trackingNumber && (
                      <div className="p-3 bg-blue-50 rounded-bottom-7 border border-blue-200">
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full min-w-full">
                          ...
                          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            {order.items.map((item, index) => (
                              <tr key={index}>
                                ...
                                <td className="py-3 px-3 whitespace-nowrap text-right text-sm">
                                ...
                                {order.status === "delivered" && (
                                  <td className="py-3 px-3 whitespace-nowrap text-right text-sm">
                                    <button
                                      onClick={(e) => handleReviewClick(e, item.productId)}
                                      className="bg-yellow-100 hover:bg-yellow-200 text-yellow-700 text-xs px-2 py-1 rounded-full flex items-center justify-center ml-auto"
                                    >
                          ...
                          <tfoot>
                            <tr className="bg-gray-50 dark:bg-gray-800">
                              <td
                                colSpan={order.status === "delivered" ? "4" : "3"}
                              ...
                              <td className="py-2 px-3 text-right text-sm font-medium">
                                $
                                {(order.totalPrice - order.shippingFee).toFixed(
                                  2
                                )}
                              </td>
                            </tr>
                            ...
                            <tr className="bg-gray-50 dark:bg-gray-800">
                              <td
                                colSpan={order.status === "delivered" ? "4" : "3"}
                                className="py-2 px-3 text-right text-sm font-bold"
                              >
                                Total:
                              </td>
